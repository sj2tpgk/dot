...
def rc_py_main():
    ...
    if not hasattr(fm, "ENV_SHELL_SAVE"): # this is for reload_config to not revert S key shell to /bin/sh
        fm.ENV_SHELL_SAVE = os.environ["SHELL"]
    ...
    # Expose scope.sh path
    os.environ["RANGER_SCOPE"] = fm.relpath('data/scope.sh')

    # Rifle config (alternatively, you can add advice to Rifle.reload_config to load the 3 files)
    import re
    def my_rifle_generage(*_):
        def canon(line:str): # not perfect but ok
            # line must contain a "="
            a1, a2 = line.split("=", 1)
            a3 = [re.sub(r"\s+", " ", x.strip()) for x in a1.split(",")]
            a4 = a2.strip()
            return ", ".join(a3) + " = " + a4
        confdir = os.path.join(os.environ.get("XDG_CONFIG_HOME", os.path.expanduser("~/.config")), "ranger")
        seen = {}
        with open(os.path.join(confdir, "rifle.conf"), "w") as fout:
            fout.write("# vim: ft=cfg\n\n### Automatically generated by rc.py, do not edit!\n\n")
            for path in [
                    ...
                    with open(path, "r") as fin:
                        for line in fin:
                            ...
                            if c not in seen:
                                fout.write(line)
                                seen[c] = 1
    advice_add(ranger.ext.rifle.Rifle, "reload_config", "before", my_rifle_generage)
    ...
    execute(f"map S shell {fm.ENV_SHELL_SAVE}")
